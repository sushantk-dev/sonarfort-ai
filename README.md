...
\# Copy your cert into the running container

docker cp certs\\SecR46.crt sonarfort-ai:/usr/local/share/ca-certificates/SecR46.crt



\# Run the fix commands inside the container

docker exec sonarfort-ai update-ca-certificates



docker exec sonarfort-ai bash -c "cat /usr/local/share/ca-certificates/SecR46.crt >> $(docker exec sonarfort-ai python3 -c 'import certifi; print(certifi.where())')"



\# Restart just the sonar-api process (not the whole container)

docker exec sonarfort-ai supervisorctl restart sonar-api

docker exec sonarfort-ai supervisorctl restart fortify-api
